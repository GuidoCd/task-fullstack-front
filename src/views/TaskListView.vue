<template>
  <div>
    ...
    <div v-else-if="!taskStore.tasks.length" class="text-center bg-white p-10 rounded-lg shadow">
      <h3 class="text-xl font-semibold text-gray-700">Aún no hay tareas</h3>
      <p class="text-gray-500 mt-2">¡Anímate a crear la primera!</p>
    </div>
    <div v-else ref="listContainer" class="space-y-4 max-h-[65vh] overflow-y-auto pr-2">
      <div v-for="task in taskStore.tasks" :key="task.id" class="bg-white p-4 rounded-lg shadow">
        ...
        <div class="mt-2 flex items-center justify-between">
          <div class="relative" v-click-outside="() => openStatusMenuId = null">
            ...
            <Teleport to="body">
              <div v-if="openStatusMenuId === task.id" :style="menuStyle"
                class="w-40 bg-white border border-gray-200 rounded-md shadow-lg z-50">
                <ul>
...
const toggleStatusMenu = (taskId: number, event: MouseEvent) => {
  if (openStatusMenuId.value === taskId) {
    // If it's already open, just close it
    openStatusMenuId.value = null;
  } else {
    // If opening, calculate the position first
...
const getTagColor = (tag: string): string => {
  ...
  return colors[tag] ?? 'bg-gray-400'; // Devuelve un color por defecto si la etiqueta no coincide
}
...
const openEditModal = (task: Task) => {
  ...
  isModalVisible.value = true;
  scrollPosition.value = listContainer.value?.scrollTop ?? 0;
};
...
const onTaskUpdated = async () => {
  closeModal(); // Primero cierra el modal
